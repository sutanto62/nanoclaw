...
import { Receipt, RegisteredGroup } from './types.js';
  ...
  switch (data.type) {
    case 'save_receipt': {
      ...
      if (
        ...
        // Resolve chat_jid from the registered group for this folder
        const groupEntry = Object.entries(registeredGroups).find(
          ([, g]) => g.folder === sourceGroup,
        );
        const chatJid = r.chat_jid || groupEntry?.[0] || sourceGroup;
