...
import { ASSISTANT_NAME, DIGEST_INTERVAL_MS, DIGEST_LOOKBACK_HOURS, GROUPS_DIR } from './config.js';
import { getAllChats, getAllRegisteredGroups, getMessagesSince, getNewMessages } from './db.js';
...
function buildLarkDigest(
  ...
  if (isMain) {
    ...
    const { messages } = getNewMessages(allLarkJids, since, ASSISTANT_NAME, 500);
    ...
    for (const [jid, msgs] of byChat) {
      ...
      for (const msg of msgs) {
        lines.push(`- [${formatTimestamp(msg.timestamp)}] ${msg.sender_name}: ${msg.content}`);
      }
      lines.push('');
    }
  } else {
...
function writeCacheFile(groupFolder: string, channel: string, content: string): void {
...
function runDigest(getGroups: () => Record<string, RegisteredGroup>): void {
  ...
  for (const [jid, group] of Object.entries(groups)) {
    try {
      // Lark digest
      const larkContent = buildLarkDigest(jid, group, since, allLarkJids, chatNames);
      if (larkContent) {
      ...
      logger.warn({ folder: group.folder, err }, 'Digest runner error for group');
    }
  }
}
